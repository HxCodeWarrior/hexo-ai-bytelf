module.exports = function strip(content, title, config) {
  ...
  if (Array.isArray(ignoreRules) && ignoreRules.length > 0) {
    ...
    if (logLevel >= 1) {
      console.warn('[Hexo-AI-ByteElf] ignore_rules 未设置或无效，跳过处理');
    }
  }

  // 在 NORMAL 或 VERBOSE 日志等级时输出原始字符串长度
  if (logLevel >= 1) {
    console.log('[Hexo-AI-ByteElf] 原始字符串长度：', content.length);
  }
  ...
  if (logLevel >= 1) {
    console.log('[Hexo-AI-ByteElf] 最终输出长度：', final.length);
  }
